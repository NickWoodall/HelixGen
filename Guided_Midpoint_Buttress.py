...
def generate_ref_helix_npz():
    ...
    X = ep
    np.savez_compressed('data/ep_for_gmp.npz', ep=X)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Guided Assembly of a Helical Protein on an input Guide. Default Generated Circle line ")

    parser.add_argument("-i","--input_endpoint", help="Input first two helices, will randomly sample from reference dataset as default",
                       default=f'data/ep_for_gmp.npz')
    parser.add_argument("-g","--guide_points", help="Input guide points, defaults to quarter circle line. Input as .npz with extension",
                       default=f'default')
    ...
    if args.input_endpoint == 'data/ep_for_gmp.npz':
        if os.path.exists('data/ep_for_gmp.npz'):
            pass
        else:
            print('one time generating reference initial helices from reference Fits_4H.csv')
            generate_ref_helix_npz()
    ...
    if args.guide_points == 'default':
        guide_points = ci_line
    else:
        rr = np.load(args.guide_points, allow_pickle=True)
        guide_points = [rr[f] for f in rr.files][0]
